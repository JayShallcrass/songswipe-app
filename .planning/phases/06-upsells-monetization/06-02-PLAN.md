...
<task type="auto">
  ...
  <action>
    1. Modify `src/app/generate/[orderId]/page.tsx`:
       ...
       - VariantSwiper does NOT currently have an onIndexChange prop. Add it to `src/components/generation/VariantSwiper.tsx`: add optional prop onIndexChange?: (index: number, total: number) => void to VariantSwiperProps interface. Call onIndexChange(newIndex, variants.length) inside goToNext, goToPrevious, and the dot indicator onClick handler whenever currentIndex changes. This allows the generation page to detect when the user has viewed all variants.
