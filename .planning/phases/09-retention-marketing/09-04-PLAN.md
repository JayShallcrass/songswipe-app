...
<task type="auto">
  ...
  <action>
    ...
    **PersonalizationForm (src/components/forms/PersonalizationForm.tsx):**
    1. Add `occasionDate: string` to the EXPORTED PersonalizationData interface (line 6-11, after `thingsToAvoid`). This interface is exported and consumed by src/app/customize/page.tsx via the onSubmit callback. Empty string = not provided.
    2. Add state: `const [occasionDate, setOccasionDate] = useState('')` (after line 29 where thingsToAvoid state is declared)
    ...
    4. In the handleSubmit function (lines 47-56), add `occasionDate` to the onSubmit call data object. Specifically, modify the onSubmit call (lines 50-55) to include:
       ```typescript
       onSubmit({
         recipientName,
         yourName,
    ...
    **Customize page (src/app/customize/page.tsx):**
    ...
    ```
    Add this after the `thingsToAvoid` line in the body object (around line 84). The handleFormSubmit function receives PersonalizationData via the onSubmit callback from PersonalizationForm.tsx and will include the occasionDate field.
  </action>
  <verify>
